...
async def telegram_passenger(passenger_data: TelegramPassengerCreate, db: AsyncSession = Depends(get_db)):
    passenger_service = PassengerService(passenger_orm=PassengerORMRepository(db=db))

    passenger: Passenger = await passenger_service.create_or_get_passenger(passenger_data=passenger_data)

    return APIResponse(
        status=APIStatusEnum.SUCCESS,
        message=f'Passenger authenticated successfully',
        detail=TelegramPassengerResponse.model_validate(passenger),
    )
